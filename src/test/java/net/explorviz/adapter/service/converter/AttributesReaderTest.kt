...
class AttributesReaderTest {
  ...
  fun testFqnFromNameReadOut() {
    ...
    assertEquals(
      DefaultAttributeValues.DEFAULT_CLASS_FQN + "." + methodName, reader.methodFqn
    )
    ...
    reader = AttributesReader(span)
    assertEquals(
      "$innerPackage.$className.$methodName", reader.methodFqn
    )
  ...
  private fun generateValidAttributesMap(): MutableList<KeyValue> {
    return mutableListOf(
      KeyValue.newBuilder().setKey(KEY_LANDSCAPE_TOKEN)
        .setValue(AnyValue.newBuilder().setStringValue(TOKEN).build()).build(),
      KeyValue.newBuilder().setKey(KEY_LANDSCAPE_SECRET)
        .setValue(AnyValue.newBuilder().setStringValue(SECRET).build()).build(),
      KeyValue.newBuilder().setKey(KEY_GIT_COMMIT_CHECKSUM)
        .setValue(AnyValue.newBuilder().setStringValue(GIT_COMMIT_CHECKSUM).build()).build(),
      KeyValue.newBuilder().setKey(KEY_HOST_NAME)
        .setValue(AnyValue.newBuilder().setStringValue(HOSTNAME).build()).build(),
      KeyValue.newBuilder().setKey(KEY_HOST_IP)
        .setValue(AnyValue.newBuilder().setStringValue(HOST_IP).build()).build(),
      KeyValue.newBuilder().setKey(KEY_APPLICATION_NAME)
        .setValue(AnyValue.newBuilder().setStringValue(APP_NAME).build()).build(),
      KeyValue.newBuilder().setKey(KEY_APPLICATION_INSTANCE_ID)
        .setValue(AnyValue.newBuilder().setStringValue(APP_INSTANCE_ID).build()).build(),
      KeyValue.newBuilder().setKey(KEY_APPLICATION_LANGUAGE)
        .setValue(AnyValue.newBuilder().setStringValue(APP_LANG).build()).build(),
      KeyValue.newBuilder().setKey(KEY_METHOD_FQN)
        .setValue(AnyValue.newBuilder().setStringValue(FQN).build()).build(),
      KeyValue.newBuilder().setKey(KEY_K8S_POD_NAME)
        .setValue(AnyValue.newBuilder().setStringValue(K8S_POD_NAME).build()).build(),
      KeyValue.newBuilder().setKey(KEY_K8S_NAMESPACE)
        .setValue(AnyValue.newBuilder().setStringValue(K8S_NAMESPACE).build()).build(),
      KeyValue.newBuilder().setKey(KEY_K8S_DEPLOYMENT)
        .setValue(AnyValue.newBuilder().setStringValue(K8S_DEPLOYMENT).build()).build(),
      KeyValue.newBuilder().setKey(KEY_K8S_NODE)
        .setValue(AnyValue.newBuilder().setStringValue(K8S_NODE).build()).build()
    )
  }

  private fun removeElementAndReturnAttributesMap(
    key: String,
    map: MutableList<KeyValue>
  ): MutableList<KeyValue> {
