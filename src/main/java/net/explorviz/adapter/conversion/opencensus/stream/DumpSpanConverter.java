...
public class DumpSpanConverter {
  ...
  private void buildTopology() {
    ...
    final KStream<byte[], byte[]> opencensusSpanDumps =
        builder.stream(this.config.getInTopicOpenCensus(),
            Consumed.with(Serdes.ByteArray(), Serdes.ByteArray()));

    final KStream<byte[], byte[]> opentelemetrySpans =
        builder.stream(this.config.getInTopicOpenTelemetry(),
            Consumed.with(Serdes.ByteArray(), Serdes.ByteArray()));

    final KStream<byte[], Span> spanKStream = opencensusSpanDumps.flatMapValues(d -> {
      try {
