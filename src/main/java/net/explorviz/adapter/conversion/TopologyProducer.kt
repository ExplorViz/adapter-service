...
class TopologyProducer {
  ...
  lateinit var spansOutTopic: String
  ...
  lateinit var tokensOutTopic: String

  @Inject
  lateinit var validator: SpanValidator

  @Inject
  lateinit var spanAvroSerde: SpecificAvroSerde<net.explorviz.avro.Span>

  @Inject
  lateinit var tokenEventAvroSerde: SpecificAvroSerde<TokenEvent>

  @Inject
  lateinit var spanConverter: SpanConverterImpl
  ...
  fun buildTopology(): Topology {
    ...
    val spanStream: KStream<ByteArray, Span> = spanByteStream.flatMapValues { data ->
      try {
        ...
        ExportTraceServiceRequest.parseFrom(data).resourceSpansList.forEach { resourceSpans ->
          resourceSpans.scopeSpansList.forEach { scopeSpans ->
            spanList.addAll(scopeSpans.spansList)
          }
        }
    ...
    // Validate Spans
    val validSpanStream: KStream<ByteArray, Span> = spanStream.flatMapValues { value ->
    ...
    // BEGIN Token Stream
    builder.stream(tokensInTopic, Consumed.with(Serdes.String(), tokenEventAvroSerde))
      .filter { key, value ->
